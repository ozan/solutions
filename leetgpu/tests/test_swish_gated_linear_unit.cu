...
bool run_test(const std::vector<float>& input, const std::vector<float>& expected, const char* name) {
    int N = input.size();
    int halfN = N / 2;
    std::vector<float> h_output(halfN);
    ...
    cudaMemcpy(h_output.data(), d_output, halfN * sizeof(float), cudaMemcpyDeviceToHost);
...
int main() {
    ...
    if (all_passed) {
        std::cout << "Success! All SwiGLU tests passed." << std::endl;
    }
