...
; recognized by the DFA. Note: Although the DFA itself is finite and
; only recognizes finite-length strings it can still recognize an
; infinite set of finite-length strings. And because stack space is
; finite, make sure you don't get stuck in an infinite loop that's not
; producing results every so often!

(defn language [{:keys [states alphabet start accepts transitions]}]
...
(assert (= #{"a" "ab" "abc"}
   (set (language '{:states #{q0 q1 q2 q3}
              :alphabet #{a b c}
              :start q0
              :accepts #{q1 q2 q3}
              :transitions {q0 {a q1}
                            ...
                            q2 {c q3}}}))))
(assert (= #{"hi" "hey" "hello"}
   (set (language '{:states #{q0 q1 q2 q3 q4 q5 q6 q7}
              ...
              :transitions {q0 {h q1}
                            ...
                            q5 {l q6}
                            q6 {o q7}}}))))
(assert (= (set (let [ss "vwxyz"] (for [i ss, j ss, k ss, l ss] (str i j k l))))
   (set (language '{:states #{q0 q1 q2 q3 q4}
              :alphabet #{v w x y z}
              :start q0
              :accepts #{q4}
...
(assert (let [res (take 2000 (language '{:states #{q0 q1}
                           :alphabet #{0 1}
                           :start q0
                           :accepts #{q0}
...
(assert (let [res (take 2000 (language '{:states #{q0 q1}
  ...
  (and (every? (partial re-matches #"n*m") res)
       (= res (distinct res)))))
(assert (let [res (take 2000 (language '{:states #{q0 q1 q2 q3 q4 q5 q6 q7 q8 q9}
