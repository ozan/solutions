...
int stringcmp(const void *a, const void *b) { return strcmp(a, b); }

void test_no_matches(void) {
  char inputs[][MAX_STR_LEN] = {"hello", "world", "zombies", "pants"};

  struct Vector vin = {inputs, sizeof(inputs) / MAX_STR_LEN};
...
void test_detect_simple_anagram(void) {
  int x;
  char inputs[][MAX_STR_LEN] = {"tan", "stand", "at"};

  char outputs[][MAX_STR_LEN] = {"tan"};
...
void test_detect_anagram(void) {
  ...
  char outputs[][MAX_STR_LEN] = {"inlets"};
  int outputs_len = sizeof(outputs) / MAX_STR_LEN;
...
void test_multiple_anagrams(void) {
  ...
  char outputs[][MAX_STR_LEN] = {"gallery", "regally", "largely"};
  int outputs_len = sizeof(outputs) / MAX_STR_LEN;
...
void test_case_insensitive_anagrams(void) {
  ...
  char outputs[][MAX_STR_LEN] = {"Carthorse"};
  int outputs_len = sizeof(outputs) / MAX_STR_LEN;
  qsort(outputs, outputs_len, sizeof(char) * MAX_STR_LEN, stringcmp);
...
void test_unicode_anagrams(void) {
  ...
  char inputs[][MAX_STR_LEN] = {"ΒΓΑ", "ΒΓΔ", "γβα"};

  struct Vector vin = {inputs, sizeof(inputs) / MAX_STR_LEN};

  char outputs[][MAX_STR_LEN] = {"ΒΓΑ", "γβα"};
  int outputs_len = sizeof(outputs) / MAX_STR_LEN;
  qsort(outputs, outputs_len, sizeof(char) * MAX_STR_LEN, stringcmp);
...
void test_misleading_unicode_anagrams(void) {
  // Despite what a human might think these words different letters, the input
  // uses Greek A and B
  // while the list of potential anagrams uses Latin A and B.
  char inputs[][MAX_STR_LEN] = {"ABΓ"};
