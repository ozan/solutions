...
BOOST_AUTO_TEST_CASE(ignores_punctuation) {
  ...
  const auto actual =
      word_count::words("car : carpet as java : javascript!!&@$%^&");
...
BOOST_AUTO_TEST_CASE(count_everything_just_once) {
  const map<string, int> expected{{"all", 2},    {"the", 2}, {"kings", 2},
                                  {"horses", 1}, {"and", 1}, {"men", 1}};
  const auto actual =
      word_count::words("all the kings horses and all the kings men");
...
BOOST_AUTO_TEST_CASE(with_apostrophes_as_quotes) {
  const map<string, int> expected{{"she", 1},    {"said", 1}, {"let's", 1},
                                  {"meet", 1},   {"at", 1},   {"twelve", 1},
                                  {"o'clock", 1}};
  const auto actual =
      word_count::words("She said, 'let's meet at twelve o'clock'");
