...
BOOST_AUTO_TEST_CASE(normalize_strange_characters) {
  BOOST_REQUIRE_EQUAL(
      "splunk", crypto_square::cipher("s#$%^&plunk").normalize_plain_text());
}

BOOST_AUTO_TEST_CASE(normalize_numbers) {
  BOOST_REQUIRE_EQUAL(
      "123go", crypto_square::cipher("1, 2, 3 GO!").normalize_plain_text());
}
...
BOOST_AUTO_TEST_CASE(plain_text_segments_from_phrase) {
  const std::vector<std::string> expected{"neverv", "exthin", "eheart",
                                          "withid", "lewoes"};

  const auto actual =
      crypto_square::cipher("Never vex thine heart with idle woes")
          .plain_text_segments();
...
BOOST_AUTO_TEST_CASE(plain_text_segments_from_complex_phrase) {
  ...
  const auto actual =
      crypto_square::cipher("ZOMG! ZOMBIES!!!").plain_text_segments();
...
BOOST_AUTO_TEST_CASE(cipher_text_short_phrase) {
  BOOST_REQUIRE_EQUAL(
      ...
      crypto_square::cipher("Time is an illusion. Lunchtime doubly so.")
          .cipher_text());
}
