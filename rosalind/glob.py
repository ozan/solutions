...
def glob(s, t):
    memo = [GAP_PENALTY * i for i in range(len(t) + 1)]

    for si, sx in enumerate(s):
        memp, memo[0] = memo[0], (si + 1) * GAP_PENALTY
        for ti, tx in enumerate(t):
            memp, memo[ti + 1] = memo[ti + 1], max(
                blosum62(sx, tx) + memp,  # substitution
                GAP_PENALTY + memo[ti+1],  # deletion
                GAP_PENALTY + memo[ti]  # insertion
            )
...
with open('/Users/oz/Downloads/rosalind_glob.txt') as f:
