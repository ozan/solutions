...
def ba5h(s, t):
    ...
    for si, sx in enumerate(s):
        ...
        for ti, tx in enumerate(t):
            best = max(
                ((1 if sx == tx else -1) * MATCH + memo_row[ti][0], (si, ti)),
                (INDEL + memo_row[ti+1][0], (si, ti+1)),
                (INDEL + nxt[ti][0], (si+1, ti)))
            if ti == len(t) - 1 and (end_score is None or best[0] > end_score):
                end_score = best[0]
                end_loc = (si, ti)
            nxt.append(best)
        memo.append(nxt)
...
sample = """GTAGGCTTAAGGTTA
TAGATA"""

# ba5h(*sample2.split('\n'))

with open('/Users/oz/Downloads/rosalind_ba5h.txt') as f:
    print(ba5h(*f.read().rstrip().split('\n')))
